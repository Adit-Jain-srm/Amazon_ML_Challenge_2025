...
### Training the Model
```bash
python train_model.py --data_path dataset/train.csv --model_path models/
```

### Making Predictions
```bash
python predict.py --test_path dataset/test.csv --output_path test_out.csv
```
...
### Files to Submit
1. **test_out.csv**: Predictions file matching sample_test_out.csv format
2. **Documentation**: 1-page document describing our approach (using Documentation_template.md)
3. **Code**: Complete solution in zipped format
